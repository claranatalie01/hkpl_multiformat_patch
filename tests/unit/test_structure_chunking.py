"""Exercise the structure-aware crawling and ingestion pipeline.
...
import hkpl_agent.ingestion.service as ingestion_service
from hkpl_agent.ingestion.chunking import build_search_text, chunk_documents
...
from hkpl_agent.ingestion.document_types import (
    chunk_policy_for,
    resolve_record_kind,
    validate_document_type,
)
from hkpl_agent.ingestion.readers import (
    _load_docling,
...
class DeterministicReaderTests(unittest.TestCase):
    ...
    def test_docling_table_replaces_flattened_text_and_preserves_trailing_prose(self) -> None:
        ...
        with (
            patch(
                "hkpl_agent.ingestion.readers._load_or_convert_docling",
                return_value=(docling_document, Path("table.json")),
            ),
            patch(
                "hkpl_agent.ingestion.readers.get_embedding_tokenizer",
    ...
    def test_docling_failure_is_not_silently_reparsed(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch(
                "hkpl_agent.ingestion.readers._load_docling",
    ...
    def test_empty_docling_html_uses_text_fallback_but_form_shell_stays_empty(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch("hkpl_agent.ingestion.readers._load_docling", return_value=[]):
